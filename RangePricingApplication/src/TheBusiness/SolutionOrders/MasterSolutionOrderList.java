...
public class MasterSolutionOrderList {
    ...
    public List<SolutionOffer> getNegotiatedSolutions(MarketChannelAssignment marketChannelCombo) {
        List<SolutionOffer> negotiatedSolutions = new ArrayList<>();

        for (SolutionOrder so : solutionorderlist) {
            MarketChannelAssignment mcc = so.getMarketChannelCombo();
            if (mcc.equals(marketChannelCombo)) {
                SolutionOffer solutionOffer = so.getSolutionOffer();
                negotiatedSolutions.add(solutionOffer);
            }
        }

        return negotiatedSolutions;
    }
}
